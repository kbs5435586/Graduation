...
void CCamera::Invalidate_IViewProjMatrix()
{
	m_ImatView = m_pTransform->Get_Matrix_Inverse();
	_matrix matOrtho = XMMatrixOrthographicLH((_float)WINCX, (_float)WINCY, 0.f, 1.f);
	CCamera_Manager::GetInstance()->SetIMatView(m_ImatView);
	CCamera_Manager::GetInstance()->SetIMatProj(m_ImatProj);
	CCamera_Manager::GetInstance()->SetIMatOrtho(matOrtho);
}
