...
_int CWire::Update_GameObject(const _float& fTimeDelta) // 서버 보낼값 결과값
{
	...
	if ((GetAsyncKeyState('1') & 0x8000) && true == pServer->Get_SelectPlayer())
	{
		pServer->send_npc_act_packet(DO_ATTACK);
	}
	if ((GetAsyncKeyState('2') & 0x8000) && true == pServer->Get_SelectPlayer())
	{
		pServer->send_npc_act_packet(DO_DEFENCE);
	}
	if ((GetAsyncKeyState('3') & 0x8000) && true == pServer->Get_SelectPlayer())
	{
		pServer->send_npc_act_packet(DO_HOLD);
	}
	if ((GetAsyncKeyState('4') & 0x8000) && true == pServer->Get_SelectPlayer())
	{
		pServer->send_npc_act_packet(DO_FOLLOW);
	}
	if ((GetAsyncKeyState('5') & 0x8000) && true == pServer->Get_SelectPlayer())
	{
		pServer->send_npc_act_packet(DO_RANDMOVE);
	}
	if ((GetAsyncKeyState('6') & 0x8000) && true == pServer->Get_SelectPlayer())
	{
	...
	if (GetAsyncKeyState('M') & 0x8000 && true == pServer->Get_SelectPlayer())
	{
		duration<double> cool_time = duration_cast<duration<double>>(high_resolution_clock::now()
	...
	if (GetAsyncKeyState('E') & 0x8000 && true == pServer->Get_SelectPlayer())
	{
		duration<double> cool_time = duration_cast<duration<double>>(high_resolution_clock::now()
	...
	if (GetAsyncKeyState('T') & 0x8000 && true == pServer->Get_SelectPlayer())
	{
		pServer->send_move_packet(GO_FORWARD);
	}
	if (GetAsyncKeyState('F') & 0x8000 && true == pServer->Get_SelectPlayer())
	{
		pServer->send_move_packet(GO_LEFT);
	}
	if (GetAsyncKeyState('G') & 0x8000 && true == pServer->Get_SelectPlayer())
	{
		pServer->send_move_packet(GO_BACK);
	}
	if (GetAsyncKeyState('H') & 0x8000 && true == pServer->Get_SelectPlayer())
	{
		pServer->send_move_packet(GO_RIGHT);
	}
	if (GetAsyncKeyState('O') & 0x8000 && true == pServer->Get_SelectPlayer())
	{
		pServer->send_rotate_packet(TURN_LEFT);
	}
	if (GetAsyncKeyState('P') & 0x8000 && true == pServer->Get_SelectPlayer())
	{
