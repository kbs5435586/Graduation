...
void CMyRect::Render_GameObject()
{
	...
	m_pShaderCom->SetUp_OnShader(m_pConstBuffer, matWorld, matView, matProj, tMainPass);
	memcpy_s(m_pData, m_iPassSize, (void*)&tMainPass, sizeof(tMainPass));

	CDevice::GetInstance()->GetCmdLst()->SetGraphicsRootConstantBufferView(0, m_pConstBuffer->GetGPUVirtualAddress());
...
HRESULT CMyRect::CreateInputLayout()
{
	...
	vecDesc.push_back(D3D12_INPUT_ELEMENT_DESC{ "COLOR", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 12, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 });
...
HRESULT CMyRect::Ready_Component()
{
	...
	m_pBufferCom = (CBuffer_RectCol*)pManagement->Clone_Component((_uint)SCENEID::SCENE_LOGO, L"Component_Buffer_RectCol");
	NULL_CHECK_VAL(m_pBufferCom, E_FAIL);
	if (FAILED(Add_Component(L"Com_Buffer", m_pBufferCom)))
		return E_FAIL;

	m_pShaderCom = (CShader*)pManagement->Clone_Component((_uint)SCENEID::SCENE_LOGO, L"Component_Shader_Default");
