...
CBuffer_Circle::CBuffer_Circle()
	: CVIBuffer()
{
}

CBuffer_Circle::CBuffer_Circle(const CBuffer_Circle& rhs)
	: CVIBuffer(rhs)
{
}

HRESULT CBuffer_Circle::Ready_VIBuffer()
{
	//정점 개수
	m_iNumVertices = 11;
...
CBuffer_Circle* CBuffer_Circle::Create()
{
	CBuffer_Circle* pInstance = new CBuffer_Circle();

	if (FAILED(pInstance->Ready_VIBuffer()))
	{
		MessageBox(0, L"CBuffer_Circle Created Failed", L"System Error", MB_OK);
		Safe_Release(pInstance);
	}
	return pInstance;
}

CComponent* CBuffer_Circle::Clone_Component(void* pArg)
{
	return new CBuffer_Circle(*this);
}

void CBuffer_Circle::Free()
{
	CVIBuffer::Free();
}
