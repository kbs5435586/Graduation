...
HRESULT CUAVManager::Ready_UAVManager()
{
	...
	pUAV = CUAV::Create(100.f, 100.f, DXGI_FORMAT_R8G8B8A8_UNORM, CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE
		, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
	m_mapUAV.insert({ L"UAV_HP", pUAV });

	pUAV = CUAV::Create(300.f, 300.f, DXGI_FORMAT_R8G8B8A8_UNORM, CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE
		, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
	m_mapUAV.insert({ L"UAV_MAX_TIME", pUAV });


	pUAV = CUAV::Create(5, 5, DXGI_FORMAT_R8G8B8A8_UNORM, CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE
		, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
	m_mapUAV.insert({ L"UAV_WoL", pUAV });

	pUAV = CUAV::Create(5, 5, DXGI_FORMAT_R8G8B8A8_UNORM, CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE
		, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
	m_mapUAV.insert({ L"UAV_WoL_Blue", pUAV });
	return S_OK;
}
