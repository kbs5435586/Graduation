...
CSufferer::CSufferer(const CSufferer& rhs)
	: CGameObject(rhs)
{
}

HRESULT CSufferer::Ready_Prototype()
{
	return S_OK;
}

HRESULT CSufferer::Ready_GameObject(void* pArg)
{
	m_IsClone = true;
...
_int CSufferer::LastUpdate_GameObject(const _float& fTimeDelta)
{
...
void CSufferer::Render_GameObject()
{
...
HRESULT CSufferer::CreateInputLayout()
{
...
CSufferer* CSufferer::Create()
{
	CSufferer* pInstance = new CSufferer();

	if (FAILED(pInstance->Ready_Prototype()))
	{
		MessageBox(0, L"CSufferer Created Failed", L"System Error", MB_OK);
...
HRESULT CSufferer::Ready_Component()
{
	CManagement* pManagement = CManagement::GetInstance();
	...
	m_pMeshCom = (CMesh*)pManagement->Clone_Component((_uint)SCENEID::SCENE_STATIC, L"Component_Mesh_Sufferer");
