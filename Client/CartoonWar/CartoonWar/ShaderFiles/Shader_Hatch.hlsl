...
VS_OUT	VS_Main(VS_IN vIn)
{
	...
	vOut.vNormal = normalize(float4(vIn.vNormal, 0.f));
...
PS_OUT	PS_Main(VS_OUT vIn)
{
	...
	float4 vLightDir = tLight[0].vLightPos - vIn.vPosition;
	vLightDir = normalize(vLightDir);

	float factor = (dot(vIn.vNormal, vLightDir) + 1.0f) * 3.0f;


	float3 vWeight123 = float3(1.0f - clamp(abs(factor - 5.0f), 0.0f, 1.0f),
		1.0f - clamp(abs(factor - 4.0f), 0.0f, 1.0f),
		1.0f - clamp(abs(factor - 3.0f), 0.0f, 1.0f));


	float3 vWeight456 = float3(1.0f - clamp(abs(factor - 2.0f), 0.0f, 1.0f),
