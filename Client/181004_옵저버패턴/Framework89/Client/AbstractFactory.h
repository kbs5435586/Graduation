...
{
public:
	static CObj* CreateEffect(const wstring& wstrStateKey, const FRAME& tFrame)
	{
		// 掘⑷類
		...
		// 蹺鼻類
		CObj* pEffect = CAbstractFactory<EFFECT>::CreateObj();
		dynamic_cast<EFFECT*>(pEffect)->SetBridge(pAnim);

		return pEffect;
	}

	static CObj* CreateEffect(const wstring& wstrStateKey, const float& fTime)
	{
		// 掘⑷類
