...
{
	CObj* pObj = CAbstractFactory<CHpBar>::CreateObj();
	GET_INSTANCE(CObjMgr)->AddObject(pObj, CObjMgr::UI);
	GET_INSTANCE(CDataSubject)->AddData(CDataSubject::PLAYER_DATA, &m_tStat);
}
...
void CPlayer::KeyInput()
{
	...
	{
		m_tStat.iHp -= 100;
		GET_INSTANCE(CDataSubject)->Notify(CDataSubject::PLAYER_DATA, &m_tStat);
	}
	...
	if (GET_INSTANCE(CKeyMgr)->KeyUp(KEY_LEFT))
	{
		CObj* pEffect = CEffectFactory<CBuffEffect, CNonAnimBridge>::CreateEffect(
			L"Fire", 5.f);
		NULL_CHECK_VOID(pEffect);
